...
Add `swift-google-api-apikeys-v2` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-api-apikeys-v2.git --from 0.1.0-preview
```

Then add `GoogleApiApiKeysV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleApiApiKeysV1 <target-name> --package swift-google-api-apikeys-v2
```
